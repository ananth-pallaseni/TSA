...
class ModelSpace():
	...
	"""
	def __init__(self, max_parents, num_interactions, num_nodes, max_order, time_scale, topology_fn):
...
def generate_models(model_space, target, true_data):
	""" Generate all possible network topologies concerning one target species
		...
		true_data - A numpy array containing the value of each species across all time stpes, such that true_data[t, s] = value of species s at time t.
	...
	for num_parents in range(1, model_space.max_parents+1):
		...
		for parent_comb in all_parent_combs:
			all_interaction_perms = model_space.__enumerate_inter_perms(num_parents, interactions)
			...
			for interaction_perm in all_interaction_perms:
				...
				for order in range(model_space.max_order+1):
					...
					dX, param_len, bounds = model_space.topology_fn(topology=topology, specie_vals=true_data)
...
def find_best_models(models, target, true_data, num_best_models=10):
	""" Outputs the model topologies for a certain target that produce data close to its "true" values. 
		...
		true_data - The "true" values of all species for all time steps. Should be a numpy array such that true_data[t, s] is the value of species s at time t. 
	...
	for (dX, param_len, bounds, top) in models:
		...
		model_details = (topology, dX, param_len, opt_params, dist)
...
def TSA(model_space, accepted_model_fn, time_scale, initial_vals):
	...
	"""
	true_data = sim_data(accepted_model_fn, time_scale, initial_vals)
